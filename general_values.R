data_by_id <- spectator1712 %>% group_by(id_secondary, title) %>% dplyr::summarise(sum_length = sum(length), median_length = median(length), mean = mean(length), n=n())
...
pure_spectator_reused <- spectator1712 %>%
