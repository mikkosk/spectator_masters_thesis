...
calc_reused_num <- function(vols, ylim, vols_name, group_name, decadeStart = NULL, decadeEnd = NULL) {
 ...
 allNumbersList <- list()
  ...
  for(i in 1:8) {
    ...
    file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition , "/number_hits/", vols_name, "/", group_name, "/volume_", i, "_hit_distinct_title.png", sep="")
  ...
  file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition , "/number_hits/", vols_name, "/all_", group_name, ".png", sep="")
  png(file=file,width=1200, height=700)
  print(hits_figs)
  dev.off()
...
christianity_vols <- lapply(volumes_array, function(x) filter(x, estc_id %in% christianity))
milton_vols <- lapply(volumes_array, function(x) filter(x,estc_id %in% milton))
instructions_vols <- lapply(volumes_array, function(x) filter(x,estc_id %in% instructions))
collections_vols <- lapply(volumes_array, function(x) filter(x, estc_id %in% collections))
misc_vols <- lapply(volumes_array, function(x) filter(x,estc_id %in% misc))
...
christianity_vols_filtered <- lapply(filtered_volumes_array, function(x) filter(x, estc_id %in% christianity))
milton_vols_filtered <- lapply(filtered_volumes_array, function(x) filter(x,estc_id %in% milton))
instructions_vols_filtered <- lapply(filtered_volumes_array, function(x) filter(x,estc_id %in% instructions))
collections_vols_filtered <- lapply(filtered_volumes_array, function(x) filter(x, estc_id %in% collections))
misc_vols_filtered <- lapply(filtered_volumes_array, function(x) filter(x,estc_id %in% misc))
...
christianityNumbers <- calc_reused_num(christianity_vols, 150, "all_clusters", "christianity")
miltonNumbers <- calc_reused_num(milton_vols, 3000, "all_clusters", "milton")
instructionsNumbers <- calc_reused_num(instructions_vols, 750, "all_clusters", "instructions")
collectionsNumbers <- calc_reused_num(collections_vols, 150, "all_clusters", "collections")
miscNumbers <- calc_reused_num(misc_vols, 150, "all_clusters", "misc")
...
christianityNumbersFiltered <- calc_reused_num(christianity_vols_filtered, 150, "filtered_clusters", "christianity")
miltonNumbersFiltered <- calc_reused_num(milton_vols_filtered, 3000, "filtered_clusters", "milton")
instructionsNumbersFiltered <- calc_reused_num(instructions_vols_filtered, 750, "filtered_clusters", "instructions")
collectionsNumbersFiltered <- calc_reused_num(collections_vols_filtered, 150, "filtered_clusters", "collections")
miscNumbersFiltered <- calc_reused_num(misc_vols_filtered, 150, "filtered_clusters", "misc")
...
#JAKAUMA KUVAAJA
numberHitDistribution <- noPureNumbersFiltered %>% arrange(desc(number_hits)) %>% mutate(row = row_number())
distributionFig <- ggplot(numberHitDistribution) + geom_area(aes(x=row, y=number_hits)) 
file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition , "/reused_distribution", ".png", sep="")
png(file=file,width=1200, height=700)
