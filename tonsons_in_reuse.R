...
## Vertaa Tonsoneiden uudelleenkäyttöä kaikkiin muihin julkaisijoihin (joilla enemmän kuin X julkaisua)
## Tsekkaa miten paljon Spec yms käyttää Tonsonseiden materiaalia ennen
## Joku verkostohomma ehkä?
## 
...
spec_scat_fig <- ggplot(biggest_publishers_spec %>% filter(w_tonson_per != 0), aes(x=w_tonson_per, y=percentage_in_reuse)) + geom_point() +xlim(0, 20)
print(spec_scat_fig)

spec_scat_reuse_mass_fig <- ggplot(biggest_publishers_spec %>%
  filter(!actor_id %in% tonson_actor_ids$actor_id) %>% 
  filter(w_tonson_per != 0) %>% mutate(reuse_mean = length_sum  / books_reusing), aes(x=w_tonson_per, y=reuse_mean)) + geom_point() +xlim(0, 20) + ylim(0, 2e+05)
...
chap3_fig_1 <- ggplot() + geom_line(data = tonson_reuse_by_decade, aes(x=publication_decade, y=books)) + xlim(1720, 1760)
print(chap3_fig_1)
...
file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition, "/tonson_reuse_books_by_decade.png", sep="")
png(file=file,width=1200, height=700)
print(chap3_fig_1)
dev.off()

file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition, "/spectator_scatter_fig.png", sep="")
png(file=file,width=1200, height=700)
print(spec_scat_fig)
dev.off()

file <- paste("C:/Users/mikko/OneDrive/Työpöytä/Gradu/masters_thesis_spectator/graphs/", currentEdition, "/spectator_scatter_fig_reuse_mass.png", sep="")
png(file=file,width=1200, height=700)
print(spec_scat_reuse_mass_fig)
dev.off()
